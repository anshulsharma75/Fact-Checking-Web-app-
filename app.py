...
def generate_pdf_report(doc_name, claims_data, verified_count, inaccurate_count, false_count, trust_score):
    ...
    h2_style    = ParagraphStyle("h2", fontName="Helvetica-Bold", fontSize=13,
                                  textColor=LIGHT, spaceAfter=6, spaceBefore=10)
    ...
    claim_style = ParagraphStyle("claim", fontName="Helvetica-Bold", fontSize=9,
                                  textColor=LIGHT, leading=13)
    correct_style = ParagraphStyle("correct", fontName="Helvetica-Oblique", fontSize=8.5,
                                    textColor=ACCENT, leading=12)

    # Header
    elements.append(Paragraph("FactLens", title_style))
    elements.append(Paragraph("AI-Powered Fact Verification Report", sub_style))
    ...
    meta_data = [
        ["Document", doc_name],
        ["Generated", now],
        ["Total Claims Analyzed", str(len(claims_data))],
    ]
    ...
    stats_data = [
        ["TRUST SCORE", "✓ VERIFIED", "⚠ INACCURATE", "✗ FALSE"],
        [f"{trust_score}% — {trust_label}",
         str(verified_count), str(inaccurate_count), str(false_count)],
    ]
    stats_table = Table(stats_data, colWidths=[60*mm, 40*mm, 40*mm, 35*mm])
    ...
    for i, r in enumerate(claims_data):
        ...
        verdict   = verif.get("verdict", "False")
        conf      = verif.get("confidence", 0)
        explain   = verif.get("explanation", "")
        correct   = verif.get("correct_fact", "")
        sources   = verif.get("sources", [])
        category  = claim.get("category", "other").upper()
        ...
        header_data = [[
            Paragraph(f"#{i+1}  {category}", small_style),
            Paragraph(f"{v_icon} {verdict.upper()}    Confidence: {conf}%",
                      ParagraphStyle("vh", fontName="Helvetica-Bold", fontSize=9,
                                     textColor=v_color, alignment=TA_RIGHT))
        ]]
        ht = Table(header_data, colWidths=[80*mm, 95*mm])
        ...
        body_rows = [
            ...
            [Paragraph(explain, small_style)],
        ]
        if correct and correct != "Claim is accurate.":
            body_rows.append([Paragraph(f"Correct fact: {correct}", correct_style)])
        if sources:
            src_text = "  •  ".join([s.get("title","")[:55] for s in sources if s.get("title")])
            if src_text:
                body_rows.append([Paragraph(f"Sources: {src_text}", small_style)])

        bt = Table([[row[0]] for row in body_rows], colWidths=[175*mm])
    ...
    elements.append(Paragraph(
        "Generated by FactLens — AI-powered fact verification | Built for CogCulture Assessment",
...
# ─── Page Config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="FactLens – AI Claim Verifier",
    page_icon="🔍",
    layout="wide",
...
<style>
    ...
    /* ── Hero banner ── */
    .hero {
        background: linear-gradient(135deg, #010409 0%, #0d1117 40%, #0c1929 100%);
    ...
    .stat-lbl  { font-size: 0.78rem; color: #8b949e; margin-top: 0.3rem; letter-spacing: 0.5px; }
...
with st.sidebar:
    ...
    # Navigation
    st.markdown("""
    <div style="font-family:'Syne',sans-serif; font-size:0.7rem; letter-spacing:2px; text-transform:uppercase; color:#8b949e; margin-bottom:0.6rem;">
        Navigation
    </div>
    """, unsafe_allow_html=True)
    ...
    max_claims = st.slider("Max claims to verify", 3, 15, 8, help="More claims = more API calls & time")
    ...
    <div style="font-size:0.75rem; color:#8b949e; line-height:1.7;">
        Built for <b style="color:#c9d1d9;">CogCulture</b><br>
        PM Trainee Assessment
    </div>
    """, unsafe_allow_html=True)
...
if page == "✨  Features":
    ...
    features = [
        ...
        ("📄", "Professional PDF Report", "Download a fully formatted PDF report with all verdicts, explanations, sources, and the trust score — ready to share."),
    ]
    for i, (icon, title, desc) in enumerate(features):
        col = f1 if i % 2 == 0 else f2
        with col:
            st.markdown(f"""
            <div class="claim-card verified" style="border-left-color:#00d4ff; margin-bottom:1rem;">
                <div style="font-size:1.8rem; margin-bottom:0.5rem;">{icon}</div>
                <div style="font-family:'Syne',sans-serif; font-size:1rem; font-weight:700; color:#f0f6fc; margin-bottom:0.4rem;">{title}</div>
...
# ── DOWNLOAD REPORT PAGE ──
    ...
    <div class="hero">
        ...
        <p class="hero-sub">Run a fact-check analysis first from the Home page, then come back here to download your report.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if "last_results" in st.session_state:
        ...
        with col1:
            st.download_button(
                ...
                data=r["pdf_bytes"],
                file_name=f"FactLens_Report_{r['doc_name'].replace('.pdf','')}.pdf",
        ...
        with col2:
            st.download_button(
                ...
                data=r["json_str"],
                file_name=f"FactLens_Data_{r['doc_name']}.json",
        ...
        <div class="empty-state">
            ...
            <div class="empty-sub">Go to <b>Home</b> in the sidebar, upload a PDF and run the analysis.<br>Your report will appear here automatically.</div>
        </div>
        """, unsafe_allow_html=True)
    st.stop()
...
<div class="hero">
    ...
    <div class="hero-pipeline">
        <span class="pipe-step">📄 Upload PDF</span>
        <span class="pipe-arrow">→</span>
        <span class="pipe-step">🤖 Extract Claims</span>
        <span class="pipe-arrow">→</span>
        <span class="pipe-step">🌐 Live Web Verify</span>
        <span class="pipe-arrow">→</span>
...
with col2:
    ...
    st.markdown("""
    <div class="info-grid">
        <div class="info-item"><b>📊 Statistics</b>Percentages, numbers, rates</div>
        <div class="info-item"><b>📅 Dates</b>Event years, launch dates</div>
        <div class="info-item"><b>💰 Financial</b>Revenue, market cap, growth</div>
        <div class="info-item"><b>⚙️ Technical</b>Versions, specs, rankings</div>
    </div>
    """, unsafe_allow_html=True)
...
def extract_claims(text, client, model, n):
    prompt = f"""You are a fact-checking specialist. Identify the {n} most important specific, verifiable factual claims from the text below.
...
Skip opinions, marketing fluff, or general statements without figures.

Return a JSON array of exactly {n} objects (or fewer if not enough claims):
  - "claim": the exact claim, 1-2 sentences max
...
def verify_claim(claim, search_data, client, model):
    ...
    if not results_text:
        results_text = "No web results found for this claim."

    prompt = f"""You are a rigorous fact-checker. Evaluate this claim against the web evidence.
...
- "Verified"   → accurate, supported by current evidence
- "Inaccurate" → wrong figure, outdated stat, or partially wrong
- "False"      → demonstrably wrong or no credible evidence

Return a JSON object:
  - "verdict": "Verified" | "Inaccurate" | "False"
  - "confidence": integer 1-100
  - "explanation": 2-3 sentences
  - "correct_fact": if Inaccurate/False, state the correct fact. If Verified, write "Claim is accurate."
...
def render_card(idx, claim, verification):
    ...
    correct   = verification.get("correct_fact", "")
    ...
    for s in sources:
        if s.get("url"):
            title = s.get("title","")[:55] or s["url"][:55]
            sources_html += f'<a href="{s["url"]}" target="_blank" class="source-chip">🔗 {title}</a>'

    correct_html = ""
    if correct and correct != "Claim is accurate.":
        correct_html = f'<div class="correct-box">💡 <b>Correct fact:</b> {correct}</div>'

    st.markdown(f"""
    <div class="claim-card {verdict_l}">
        <span class="verdict-badge badge-{verdict_l}">{icons.get(verdict_l,"?")} {verdict}</span>
        <span class="cat-tag">{cat_icons.get(category,"📌")} {category}</span>
        <span class="conf-badge">Confidence: {confidence}%</span>
        <div class="claim-text">#{idx+1} — {claim['claim']}</div>
...
if uploaded_file:
    ...
    if st.button("🚀 Start Fact-Check Analysis", use_container_width=True, type="primary"):
        ...
        if not pdf_text or len(pdf_text) < 100:
            st.error("❌ Could not extract meaningful text. Try a different PDF.")
            st.stop()
        ...
        with st.spinner("🤖 Identifying verifiable claims..."):
            try:
                claims = extract_claims(pdf_text, groq_client, model_choice, max_claims)
            except Exception as e:
                st.error(f"❌ Claim extraction failed: {e}")
                st.stop()

        if not claims:
            st.warning("⚠️ No specific verifiable claims found in this document.")
            st.stop()

        st.info(f"🎯 Found **{len(claims)}** verifiable claims to check")
        st.markdown("---")
        ...
        for i, claim in enumerate(claims):
            short = claim['claim'][:65]
            status_text.markdown(f"🔍 Checking claim {i+1}/{len(claims)}: *{short}...*")

            search_data  = search_web(claim.get("search_query", claim["claim"]), TAVILY_API_KEY)
            try:
                verification = verify_claim(claim["claim"], search_data, groq_client, model_choice)
            except Exception as e:
                verification = {
                    "verdict": "False", "confidence": 0,
                    "explanation": f"Verification error: {e}",
                    "correct_fact": "Could not verify.", "sources": []
                }
        ...
        boxes = [
            (c1, len(claims), "#58a6ff", "Claims Analyzed"),
            (c2, verified_count,   "#22c55e", "✓ Verified"),
            (c3, inaccurate_count, "#f59e0b", "⚠ Inaccurate"),
            (c4, false_count,      "#ef4444", "✗ False"),
        ]
        for col, num, color, label in boxes:
            with col:
        ...
        st.markdown(f"""
        <div class="trust-banner" style="background:{t_bg};border:1px solid {t_color}33;">
            <div class="trust-num" style="color:{t_color}">{trust_score}%</div>
            <div class="trust-label" style="color:{t_color}">Document Trust Score — {t_label}</div>
        </div>
        """, unsafe_allow_html=True)
            ...
            "results": [
                {
                    ...
                    "explanation": r["verification"]["explanation"],
                    "correct_fact": r["verification"]["correct_fact"],
                    "sources": r["verification"].get("sources", []),
                }
                for r in results
            ]
        }

        # Generate PDF always (save to session state for Download Report page)
        try:
            pdf_bytes = generate_pdf_report(
                uploaded_file.name, results,
                verified_count, inaccurate_count, false_count, trust_score
            )
        except Exception as e:
            pdf_bytes = None
        ...
        # Save to session state so Download Report page can access it
        ...
        with col_dl1:
            if pdf_bytes:
                st.download_button(
                    label="⬇️ Download PDF Report",
                    data=pdf_bytes,
                    file_name=f"FactLens_Report_{uploaded_file.name.replace('.pdf','')}.pdf",
                    mime="application/pdf",
                    use_container_width=True
                )
            else:
                st.error("PDF generation failed — use JSON below.")
        with col_dl2:
            st.download_button(
                label="⬇️ Download JSON Data",
                data=json_str,
                file_name=f"FactLens_Data_{uploaded_file.name}.json",
                mime="application/json",
                use_container_width=True
            )
    ...
    <div class="empty-state">
        ...
        <div class="empty-sub">
            Upload any PDF — marketing reports, research papers, press releases, whitepapers.<br>
            FactLens will extract every verifiable claim and cross-check it against live web data.
        </div>
    </div>
    """, unsafe_allow_html=True)
